...
Spectrogram.prototype.push = function (frequencies) {
	...
	frequencies = frequencies.map((v, i) => {
		//apply weighting
	...
	//choose bigger data
	var bigger = this.magnitudes.length >= frequencies.length ? this.magnitudes : frequencies;
	var shorter = (bigger === frequencies ? this.magnitudes : frequencies);
	...
	for (var i = 0; i < bigger.length; i++) {
		bigger[i] = bigger[i] * smoothing + shorter[Math.floor(shorter.length * (i / bigger.length))] * (1 - smoothing);
	}

	//save actual frequencies in db
	this.magnitudes = bigger;

	//find peak
	this.peak = bigger.reduce((prev, curr) => Math.max(curr, prev), 0);

	//emit magnitudes in db range
	this.emit('push', bigger, this.peak);
